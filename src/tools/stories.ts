...
import { date, has, is, safeInteger, safeNullableInteger, user } from "./utils/validation";
	...
	static create(client: ShortcutClientWrapper, server: CustomMcpServer) {
		...
		server.addToolWithWriteAccess(
			...
			"Update a Shortcut story. Only provide fields to update.",
			{
				...
				description: z.string().max(10_000).optional().describe("Story description"),
				type: z.enum(["feature", "bug", "chore"]).optional().describe("Story type"),
				epic: safeNullableInteger().optional().describe("Epic ID (null to unset)"),
				estimate: safeNullableInteger().optional().describe("Point estimate (null to unset)"),
				iteration: safeNullableInteger().optional().describe("Iteration ID (null to unset)"),
				owner_ids: z.array(z.string()).optional().describe("Owner user UUIDs"),
				workflow_state_id: safeInteger().optional().describe("Workflow state ID"),
				labels: z
					.array(
						z.object({
				...
				project_id: safeNullableInteger().optional().describe("Project ID (null to unset)"),
